...
def test_happy_path(sale_setup, mock_user) -> None:
    ...
    sale_setup.usecase.run(mock_user, SaleDTOMother.create(product_id=product.id, amount=3))
...
def test_cannot_register_sale_for_a_product_that_does_not_exist(sale_setup, mock_user) -> None:
    with pytest.raises(ProductNotFoundException):
        sale_setup.usecase.run(mock_user, SaleDTOMother.create(product_id="irrelevant-id", amount=2))
